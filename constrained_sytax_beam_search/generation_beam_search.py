    ...
    def step_sentence_constraint(
        ...
        for seq_idx, pre_seq in enumerate(this_batch_input_ids):
            ...
            if not advance_state.completed:
                # new look at this
                advance_tokens = torch.LongTensor(
                    advance_state.advance()).to(device)
                for advance_token in advance_tokens:
                    ...
                    advance_seq = torch.cat(
                        (pre_seq, advance_token.unsqueeze(0)), -1).cpu().tolist()
                ...
                new_score, new_token = torch.max(
                    this_batch_token_scores[seq_idx], 0)  # some next probable token
        ...
        if len(track_new["new_indices"]) > 0:
            ...
            all_banks = torch.tensor([one.get_bank()
                                     for one in all_states]).to(device)
            # new
            # all_states = track_new["new_states"]
            ...
            # print(f"rearrangers:{rearrangers}")

            # indices = indices[rearrangers][:orig_len]

            # Find indices where elements are equal to the first element
            max_bank = sorted_banks[0]
            matching_indices = torch.eq(sorted_banks, max_bank)

            # Get the actual indices
            filtered_indices = torch.nonzero(matching_indices).squeeze()

            # check if filtered indices are empty: This occurs if the sentence is just starting
            if filtered_indices.dim() == 0:
                filtered_indices = torch.tensor([0], device=device)

            # duplicate to be at least orig_len(number of beams)

            # filtered indices must be at least beam amount
            # Calculate how many times we need to repeat the last element
            rpt = max(orig_len - len(filtered_indices), 0)

            # check if need to pad
            if rpt > 0:
                # Repeat the last element of the tensor concatenate
                padding = filtered_indices[-1].repeat(rpt)
                filtered_indices = torch.cat(
                    [filtered_indices, padding], dim=0)

            indices = indices[filtered_indices][:orig_len]

            max_bank = sorted_banks.max().item()
            # print(max_bank)

            sent_beam_scores = all_scores[indices]
            sent_beam_tokens = all_tokens[indices]
            sent_beam_indices = torch.cat(
                (sent_beam_indices, new_indices))[indices]
