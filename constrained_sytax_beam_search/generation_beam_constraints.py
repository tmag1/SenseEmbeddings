...
class PhrasalConstraint(Constraint):
    ...
    def __init__(self, token_ids: List[int], alt_tokens: List[List[int]]):
        ...
        #     self.fire = special_tokens[0]

    def advance(self):
        # new
        # if self.completed:
        #     # if the phrasal constraint has been fulfilled, return None
        #     return None

        if self.fulfilled_idx + 1 >= len(self.alt_tokens):
            return None
        else:
            # return self.alt_tokens[self.fulfilled_idx + 1][0]
            return self.alt_tokens[self.fulfilled_idx + 1]

        token_list = self.alt_tokens[self.fulfilled_idx + 1]

        return self.alt_tokens[self.fulfilled_idx + 1][0]

        return self.token_ids[self.fulfilled_idx + 1]

        # target_tokens = self.alt_tokens[self.fulfilled_idx + 1]
    ...
    def does_advance(self, token_id: int):
        # new
        does = token_id in self.alt_tokens[self.fulfilled_idx + 1]
        # if (does):
        #     print("does advance")
    ...
    def copy(self, stateful=False):
        new_constraint = PhrasalConstraint(self.token_ids, self.alt_tokens)
...
class ConstraintListState:
    ...
    def init_state(self):
        ...
        self.pending_constraints = [constraint.copy(
            stateful=False) for constraint in self.constraints]
    ...
    def get_bank(self):
        ...
        # if self.completed:
        #     # extra points for having all constraints fulfilled
        #     add += self.max_seqlen * len(self.complete_constraints)
            ...
            for cidx, pending_constraint in enumerate(self.pending_constraints):
                if pending_constraint.does_advance(token_id):
                    ...
                    if complete or stepped:
                        ...
                        # prevent accidentally stepping through multiple constraints with just one token.
                        break
